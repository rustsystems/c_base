...
if test yes = yes; then
  if test 'yes' = yes; then
    ...
    LIBINTL="${relative_builddir}/intl/.libs/libintl.so -Wl,-rpath,${relative_builddir}/intl/.libs -liconv -Wl,-framework -Wl,CoreFoundation"
  else
    LIBINTL="${relative_builddir}/intl/.libs/libintl.a -liconv -Wl,-framework -Wl,CoreFoundation"
  fi
else
...
# gets found.
if test yes = yes; then
  LTLIBINTL="${relative_builddir}/intl/libintl.la -liconv -Wl,-framework -Wl,CoreFoundation"
else
  # The functionality is provided by libc.
  LTLIBINTL=
fi
